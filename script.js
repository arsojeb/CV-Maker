  ...
  language: {
    fields: [
      { key: "name", label: "Language", placeholder: "Spanish", full: false },
      {
        ...
        placeholder: "",
        full: false,
        select: ["Native", "Fluent", "Advanced", "Intermediate", "Basic"],
      },
    ],
  },
  project: {
...
function applyColorToPaper() {
  ...
  if (tpl === "modern" || tpl === "creative") {
    paper.querySelector(".cv-header, .cv-sidebar") &&
      (paper.querySelector(
        tpl === "creative" ? ".cv-sidebar" : ".cv-header",
  ...
  if (tpl === "modern") {
    css =
      '.cv-paper[data-template="modern"] .cv-title{color:' +
      currentColor +
      "!important}" +
      '.cv-paper[data-template="modern"] .cv-section-title{color:' +
      currentColor +
      "!important}" +
      '.cv-paper[data-template="modern"] .cv-skill-chip{border-color:' +
      currentColor +
      "!important;color:" +
      currentColor +
    ...
    css =
      '.cv-paper[data-template="creative"] .cv-sidebar{background:' +
      currentColor +
      "!important}" +
      '.cv-paper[data-template="creative"] .cv-main .cv-section-title{border-color:' +
      currentColor +
      "!important;color:" +
      currentColor +
      "!important}" +
      '.cv-paper[data-template="creative"] .cv-sidebar .cv-title{color:' +
      currentColor +
      "!important;opacity:1!important}";
  } else if (tpl === "executive") {
    css =
      '.cv-paper[data-template="executive"] .cv-header{border-color:' +
      currentColor +
      "!important}" +
      '.cv-paper[data-template="executive"] .cv-section-title{color:' +
      currentColor +
      "!important}" +
      '.cv-paper[data-template="executive"] .cv-section-title::after{content:"";display:inline-block;width:20px;height:2px;background:' +
      currentColor +
      ";margin-left:0}" +
      '.cv-paper[data-template="executive"] .cv-skill-chip{border-color:' +
      currentColor +
      "!important}";
  } else if (tpl === "compact") {
    css =
      '.cv-paper[data-template="compact"] .cv-header{border-color:' +
      currentColor +
      "!important}" +
      '.cv-paper[data-template="compact"] .cv-section-title{color:' +
      currentColor +
      "!important}";
  } else if (tpl === "classic") {
    css =
      '.cv-paper[data-template="classic"] .cv-section-title{color:' +
      currentColor +
    ...
    css =
      '.cv-paper[data-template="minimal"] .cv-section-title{color:' +
      currentColor +
      "!important}";
  }
...
function renderSectionManager() {
  ...
  sections.forEach(function (sec, idx) {
    ...
    item.innerHTML =
      '<button class="btn-icon" onclick="moveSection(' +
      idx +
      ...
      (idx === sections.length - 1 ? " disabled" : "") +
      '><i class="fa-solid fa-chevron-down"></i></button>' +
      '<span class="sm-name"><i class="fa-solid ' +
      sec.icon +
      '" style="margin-right:5px;color:var(--accent);font-size:10px"></i>' +
      sec.label +
      "</span>" +
  ...
  c.innerHTML = skills
    .map(function (s, i) {
      return (
        ...
        i +
        ')" aria-label="Remove"><i class="fa-solid fa-xmark"></i></button></span>'
      );
    })
    .join("");
...
function addEntry(type) {
  var id = Date.now();
  var entry = { id: id };
  entryConfigs[type].fields.forEach(function (f) {
    entry[f.key] = "";
  });
...
function removeEntry(type, id) {
  dataEntries[type] = dataEntries[type].filter(function (e) {
    return e.id !== id;
  });
  renderEntries(type);
  updatePreview();
...
function updateEntry(type, id, key, value) {
  var entry = dataEntries[type].find(function (e) {
    return e.id === id;
  });
...
function renderEntries(type) {
  ...
  container.innerHTML = dataEntries[type]
    .map(function (e) {
      var html =
        '<div class="entry-block"><button class="btn btn-danger btn-sm remove-entry" onclick="removeEntry(\'' +
        type +
        "'," +
        e.id +
        ')" aria-label="Remove"><i class="fa-solid fa-trash"></i></button><div class="form-grid">';
      ...
      config.fields.forEach(function (f) {
        ...
        if (f.textarea) {
          html +=
            ...
            f.label +
            '</label><textarea class="field-textarea" oninput="updateEntry(\'' +
            type +
            "'," +
            e.id +
            ",'" +
          ...
          html +=
            ...
            '"><label class="field-label">' +
            f.label +
            '</label><select class="field-select" onchange="updateEntry(\'' +
            type +
            "'," +
            e.id +
            ",'" +
            ...
            f.select
              .map(function (opt) {
                return (
                  ...
                  opt +
                  '"' +
                  ((e[f.key] || "Intermediate") === opt ? " selected" : "") +
                  ">" +
                  opt +
                  "</option>"
                );
          ...
          html +=
            ...
            '"><label class="field-label">' +
            f.label +
            '</label><input class="field-input" value="' +
            escA(e[f.key] || "") +
            '" oninput="updateEntry(\'' +
            type +
            "'," +
...
function updatePreview() {
  ...
  var hasContent =
    ...
    web ||
    summary ||
    Object.keys(dataEntries).some(function (k) {
      return dataEntries[k].some(function (e) {
  ...
  if (email)
    contacts.push(
      '<span><i class="fa-solid fa-envelope"></i> ' + esc(email) + "</span>",
    );
  if (phone)
    contacts.push(
      '<span><i class="fa-solid fa-phone"></i> ' + esc(phone) + "</span>",
    );
  if (loc)
    contacts.push(
  ...
  if (web)
    contacts.push(
      '<span class="cv-website"><i class="fa-solid fa-globe"></i> ' +
        esc(web) +
        "</span>",
    );
  ...
  if (photoData)
    photoHtml =
      '<div class="cv-photo"><img src="' + photoData + '" alt="Photo"></div>';
  ...
  if (isCreative) {
    ...
    sections.forEach(function (sec) {
      ...
      if (sec.id === "skills" && skills.length) {
        html +=
          '<div class="cv-section"><div class="cv-section-title">' +
          cvSectionTitles.skills +
          '</div><div class="cv-skills-list">' +
          skills
            .map(function (s) {
              return '<span class="cv-skill-chip">' + esc(s) + "</span>";
            })
            .join("") +
          "</div></div>";
      }
      if (sec.id === "languages") {
        var valid = dataEntries.language.filter(function (l) {
          return l.name;
        });
        if (valid.length) {
          html +=
            '<div class="cv-section"><div class="cv-section-title">' +
            cvSectionTitles.languages +
            '</div><div class="cv-skills-list">' +
            valid
              .map(function (l) {
                return (
      ...
      if (sec.id === "certifications") {
        var valid = dataEntries.certification.filter(function (c) {
          return c.name;
        });
        if (valid.length) {
          html +=
            '<div class="cv-section"><div class="cv-section-title">' +
            cvSectionTitles.certifications +
            "</div>";
          valid.forEach(function (c) {
            html +=
              '<div class="cv-entry"><div class="cv-entry-title" style="font-size:9.5px">' +
              esc(c.name) +
              "</div>";
            if (c.org)
              html += '<div class="cv-entry-sub">' + esc(c.org) + "</div>";
            if (c.date)
              html +=
                '<div class="cv-entry-date" style="font-size:9px">' +
                esc(c.date) +
                "</div>";
            html += "</div>";
          });
    ...
    sections.forEach(function (sec) {
      ...
      if (sec.id === "summary" && summary) {
        html +=
          '<div class="cv-section"><div class="cv-section-title">' +
          cvSectionTitles.summary +
          '</div><div class="cv-entry-desc">' +
          esc(summary) +
          "</div></div>";
      }
      if (sec.id === "experience") {
        var valid = dataEntries.experience.filter(function (e) {
          return e.title || e.company;
        });
        if (valid.length) {
          html +=
            '<div class="cv-section"><div class="cv-section-title">' +
            cvSectionTitles.experience +
            "</div>";
          valid.forEach(function (e) {
            html += buildEntryHtml(e, "title", "company");
          });
          html += "</div>";
        }
      }
      if (sec.id === "education") {
        var valid = dataEntries.education.filter(function (e) {
          return e.degree || e.school;
        });
        if (valid.length) {
          html +=
            '<div class="cv-section"><div class="cv-section-title">' +
            cvSectionTitles.education +
            "</div>";
          valid.forEach(function (e) {
            html += buildEntryHtml(e, "degree", "school");
          });
          html += "</div>";
        }
      }
      if (sec.id === "projects") {
        var valid = dataEntries.project.filter(function (e) {
          return e.name;
        });
        if (valid.length) {
          html +=
            '<div class="cv-section"><div class="cv-section-title">' +
            cvSectionTitles.projects +
            "</div>";
          valid.forEach(function (e) {
            html += buildEntryHtml(e, "name", "role");
          });
          html += "</div>";
        }
      }
    ...
    sections.forEach(function (sec) {
      ...
      if (sec.id === "summary" && summary) {
        html +=
          '<div class="cv-section"><div class="cv-section-title hf">' +
          cvSectionTitles.summary +
          '</div><div class="cv-entry-desc">' +
          esc(summary) +
          "</div></div>";
      ...
      if (sec.id === "certifications") {
        ...
        if (valid.length) {
          ...
          valid.forEach(function (c) {
            html +=
              '<div class="cv-entry"><div class="cv-entry-header"><span class="cv-entry-title">' +
              esc(c.name) +
              "</span>";
            if (c.date)
              html += '<span class="cv-entry-date">' + esc(c.date) + "</span>";
            html += "</div>";
            if (c.org)
              html += '<div class="cv-entry-sub">' + esc(c.org) + "</div>";
            html += "</div>";
...
function buildEntryHtml(e, titleKey, subKey) {
  ...
  h +=
    '<div class="cv-entry-header"><span class="cv-entry-title">' +
    esc(e[titleKey] || "Untitled") +
    "</span>";
...
function resetAll() {
  ...
  renderSkillTags();
  Object.keys(dataEntries).forEach(function (type) {
    renderEntries(type);
  });
  renderSectionManager();
  updatePreview();
    ...
    try {
      ...
      if (data.template) {
        ...
        document.querySelectorAll(".template-option").forEach(function (o) {
          o.classList.toggle(
            "active",
            o.getAttribute("data-tpl") === data.template,
          );
        });
        document
          .getElementById("cvPaper")
          .setAttribute("data-template", data.template);
      }
      if (data.color) {
      ...
      if (data.font) {
        ...
        document.querySelectorAll(".font-option").forEach(function (o) {
          o.classList.toggle(
            "active",
            o.getAttribute("data-font") === data.font,
          );
        });
